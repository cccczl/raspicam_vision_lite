...
def gen(camera, model):
    ...
    if p_active_children := mp.active_children():
        for p_ac in p_active_children:
            p_ac.terminate()
            p_ac.join()
            mplogger.info(f'Terminated child process {p_ac.name} (pid {p_ac.pid}).')

    # Initiates a new child process
    daemon_name = f'TFLiteInterpreterDaemon_{model.target}'
    p = mp.Process(name=daemon_name, target=get_inference, args=(model, frame_in_queue, label_out_queue), daemon=True)
    p.start()
    mplogger.info(f'Started child process {p.name} (pid {p.pid}).')
    ...
    while True:
        ...
        if label_text:
            ...
            for i, text in enumerate(label_text):
                FONT_COLOR = FONT_COLOR_HEADER if i == 0 else FONT_COLOR_LABEL
